...
if [ ! -f /etc/tracim/config.ini ]; then
    ...
    sed -i "s/\(^# radicale.server.host *= *\).*/radicale.server.host = 0.0.0.0/" /etc/tracim/config.ini
    sed -i "s/\(^# radicale.server.allow_origin *= *\).*/radicale.server.allow_origin = */" /etc/tracim/config.ini
